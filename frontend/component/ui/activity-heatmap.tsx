...
const ACTIVITY_COLORS_LIGHT = [
  "bg-neutral-200", // 0 - no activity
  "bg-green-200", // 1 - low
...
export function ActivityHeatmap({ data, onCelebration }: ActivityHeatmapProps) {
  ...
  const ACTIVITY_COLORS = theme === "dark" ? ACTIVITY_COLORS_DARK : ACTIVITY_COLORS_LIGHT;
  
  // Theme classes
  const cardBg = theme === "dark" ? "bg-neutral-900/80" : "bg-white";
  ...
  return (
    <div className={`${cardBg} border ${cardBorder} rounded-xl p-6`}>
      ...
      {/* Month labels */}
      <div className={`flex mb-2 ml-8 text-xs ${textMuted}`}>
        {(() => {
      ...
      <div className="flex">
        {/* Day labels */}
        <div className={`flex flex-col gap-[2px] mr-2 text-xs ${textMuted}`}>
          {DAYS.map((day, i) => (
      ...
      <AnimatePresence>
        {hoveredDay && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className={`fixed z-50 ${theme === "dark" ? "bg-neutral-800 border-neutral-700" : "bg-white border-neutral-200"} border rounded-lg p-3 shadow-xl pointer-events-none`}
            style={{
              left: tooltipPosition.x + 10,
              top: tooltipPosition.y - 80,
            }}
          >
            <p className={`${textPrimary} font-medium mb-1`}>{hoveredDay.date}</p>
...
export function StreakDisplay({ currentStreak, longestStreak, encouragement }: StreakDisplayProps) {
  ...
  const textPrimary = theme === "dark" ? "text-white" : "text-neutral-900";
  const textSecondary = theme === "dark" ? "text-neutral-400" : "text-neutral-600";
  const accentText = theme === "dark" ? "text-orange-300" : "text-orange-600";
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className={`text-3xl font-bold ${textPrimary}`}>
              {currentStreak}
              <span className={`text-lg ${textSecondary} ml-2`}>day streak</span>
            </p>
            <p className={`text-sm ${textSecondary}`}>
        ...
        <div className="hidden md:block max-w-xs">
          <p className={`${accentText} italic`}>&ldquo;{encouragement}&rdquo;</p>
        </div>
      </div>
    </motion.div>
  );
