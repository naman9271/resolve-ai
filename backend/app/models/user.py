...
class SessionDuration(str, Enum):
    """Session duration options for mentoring"""
...
class MentorProfile(Base):
    ...
    # Display Info (Name and photo will NOT be anonymous per new requirement)
    ...
    college_id_url: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)  # College ID proof
    ...
    session_rate_30_min: Mapped[int] = mapped_column(default=99)  # in INR for 30 min
    session_rate_1_hour: Mapped[int] = mapped_column(default=149)  # in INR for 1 hour
    is_available: Mapped[bool] = mapped_column(Boolean, default=True)
    available_slots: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON of available time slots
    
    # Stats
    total_sessions: Mapped[int] = mapped_column(default=0)
    total_earnings: Mapped[int] = mapped_column(default=0)
    pending_earnings: Mapped[int] = mapped_column(default=0)
...
class MentorSession(Base):
    """Track mentor-student sessions"""
    __tablename__ = "mentor_sessions"
    
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    mentor_id: Mapped[int] = mapped_column(ForeignKey("mentor_profiles.id"), index=True)
    student_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    
    # Session Details
    session_type: Mapped[SessionDuration] = mapped_column(SQLEnum(SessionDuration))
    scheduled_at: Mapped[datetime] = mapped_column(DateTime)
    duration_minutes: Mapped[int] = mapped_column()
    amount: Mapped[int] = mapped_column()  # Amount in INR
    ...
    # Rating
    student_rating: Mapped[Optional[int]] = mapped_column(nullable=True)  # 1-5
