...
class MentorProfileCreate(BaseModel):
    """Schema for creating mentor profile during registration"""
    ...
    jee_roll_number: Optional[str] = Field(None, max_length=20)
...
class MentorProfileResponse(BaseModel):
    ...
    session_rate_30_min: int
    session_rate_1_hour: int
    is_available: bool
    total_sessions: int
    rating: float
    total_reviews: int
...
class MentorDashboardResponse(BaseModel):
    """Schema for mentor dashboard data (private view)"""
    id: int
    user_id: int
    display_name: str
    profile_photo_url: Optional[str] = None
...
class MentorProfileUpdate(BaseModel):
    ...
    bio: Optional[str] = None
    expertise_subjects: Optional[str] = None
    session_rate_30_min: Optional[int] = Field(None, ge=0)
    session_rate_1_hour: Optional[int] = Field(None, ge=0)
    is_available: Optional[bool] = None
    available_slots: Optional[str] = None
    profile_photo_url: Optional[str] = None


class MentorBankingUpdate(BaseModel):
    """Schema for updating mentor banking info"""
...
class MentorVerificationRequest(BaseModel):
    """Schema for admin to verify mentor"""
    status: VerificationStatus
    rejection_reason: Optional[str] = None


class MentorSessionCreate(BaseModel):
    """Schema for creating a mentor session"""
    mentor_id: int
    session_type: str  # "30_min" or "1_hour"
    scheduled_at: datetime
    notes: Optional[str] = None


class MentorSessionResponse(BaseModel):
    """Schema for mentor session response"""
    id: int
    mentor_id: int
    student_id: int
    session_type: str
...
class MentorEarningsResponse(BaseModel):
    """Schema for mentor earnings summary"""
    total_earnings: int
    pending_earnings: int
    total_sessions: int
